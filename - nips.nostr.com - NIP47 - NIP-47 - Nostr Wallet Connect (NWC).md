---
source: "https://nips.nostr.com/47"
author:
published:
created: 2026-04-23
---
...
- [Encryption](#encryption)
	- [Request events](#request-events)
		- [Notification events](#notification-events-1)
- [Using a dedicated relay](#using-a-dedicated-relay)
- [Metadata](#metadata)
- [Appendix](#appendix)
	- [Example NIP-47 info event](#example-nip-47-info-event)
		- [Example Hold Invoice Support Flow](#example-hold-invoice-support-flow)
...
The content of requests and responses is encrypted with [NIP44](https://nips.nostr.com/44), and is a JSON-RPCish object with a semi-fixed structure.

**Important note for backwards-compatibility:** The initial version of the protocol used [NIP04](https://nips.nostr.com/4). If a **wallet service** or client app does not include the `encryption` tag in the `info` or request events, it should be assumed that the connection is using NIP04 for encryption. See the [Encryption](#encryption) section for more information.
...
The content of notifications is encrypted with [NIP44](https://nips.nostr.com/44) (or NIP-04 for legacy client apps), and is a JSON-RPCish object with a semi-fixed structure:
...
The initial version of NWC used [NIP-04](https://nips.nostr.com/4) for encryption which has been deprecated and replaced by [NIP-44](https://nips.nostr.com/44). NIP-44 should always be preferred for encryption, but there may be legacy cases where the **wallet service** or **client** has not yet migrated to NIP-44. The **wallet service** and **client** should negotiate the encryption method to use based on the `encryption` tag in the `info` event.
