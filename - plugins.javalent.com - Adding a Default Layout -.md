---
source: "https://plugins.javalent.com/statblocks/layout/add-new-default"
author:
published:
created: 2026-04-18
---
...
1. In your newly renamed folder, rename `your-layout.ts`. For example, we can choose to have this be named `13th-age-monster.ts` or `halo-top-ice-cream.ts`. For the purposes of this guide, we will still refer to it as `your-layout.ts`.
2. Let's explore the rest of the contents of this folder.
...
1. Now go ahead and fill out these folders and update your readme.
2. When you are ready to get ready to edit `your-layout.ts`, move onto the next section.
...
We suggest when you first paste your layout.json into `your-layout.ts` that you use one of the existing [Fantasy Statblocks/Integrated Layouts/Integrated Layouts](https://plugins.javalent.com/statblock/layouts/integrated) as an example. Any of them will adequately show which points of the [.json](https://plugins.javalent.com/glossary/json) file you need to copy and paste from. Above, we have included a small sample of the start point of what it looks like when you paste in your blocks.
...
1. To edit `import { LayoutName }`, change `Layoutname` to match the name you placed in your `export const` line on `your-layout.ts`. `import { YourLayoutName }`
2. To edit `from "./folder/"`, provide a relative path to `your-layout.ts` without adding the `.ts` extension. `from "./your-folder/your-layout"`
3. Similarly, repeat the same for the export line. `export * from "./your-folder/your-layout"`.
4. Finally, add `YourLayoutName` one more time in the list under `export const DefaultLayouts`.
5. Save the file.
