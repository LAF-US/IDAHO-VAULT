...
class Utilities extends \Libsyn {
	...
	public static function getUserOptionKeys() {
		return array(
				...
				'libsyn-podcasting-import_posts_data',
    ...
    /**
     * Simple method to check if a string is Json
     *
	 * @since 1.0.1.1
	...
	/**
	 * Checks to see if the post_meta value exists based on the post_meta key
	 * Note: This only checks published posts
	 *
	 * @since 1.2.2.7
	 * @param  string $postMeta_key   Post Meta Key
	 * @param  string|int $postMeta_value Post Meta Value for Key
	 * @return bool                Post Meta Value exists for key
	 */
	public function check_post_meta_exists($postMeta_key, $postMeta_value) {
		try {
			global $wpdb;
			$check = $wpdb->get_results(
			    $wpdb->prepare(
			        ...
			        ",
			        array($postMeta_key, $postMeta_value)
			    )
			);
			if ( $check ) {
				return true;
	...
	/**
	 * Method will chmod dirs and files recursively
	 *
	 * @since 1.0.1.1
	 * @param type $start_dir
	 * @param type $debug (set false if you don't want the this to echo)
	 *
	 * @return void
	 */
    ...
    /**
     * Simple method returns a pretty-print array
     *
	 * @since 1.0.1.1
     * @param array $arr
     *
