...
**[fact]** Reynolds' Boids (1986) proved that **three simple local rules**, each agent acting only on its **local neighborhood**, produce **coherent global flocking with no leader, no flight plan, no central controller.** A vault of agents is the same problem: it cannot be centrally scripted, so coherence must **emerge** from each agent running good local rules — which means each agent needs a **trustworthy local frame.**
...
## The mapping — Boids ↔ the Swarm ↔ the graph
...
| emergent flock | **emergent clusters**; orphans drift to the rim | the Deltora / ASOIAF / Keys flocks; **bridge nodes** fly between them | the **wikilinks** the agents add |
