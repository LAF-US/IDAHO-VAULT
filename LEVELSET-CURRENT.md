...
| Google Calendar | `adjunct` | `available-read-only` | `read-only` | Scheduling and availability context | Vault + GitHub + Linear | Scheduling writes are out of scope for the hub |
...
| Cloudflare | `deferred` | `plugin-available-deferred` | `deferred` | Platform and deployment lane | Vault + GitHub + Linear | Requires separate activation plan before live use |
| Hugging Face | `deferred` | `plugin-available-deferred` | `deferred` | Research and remote compute lane | Vault + GitHub + Linear | Requires separate activation plan before live use |
