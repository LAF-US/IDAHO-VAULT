---
source: "https://alignment.anthropic.com/2026/agentic-misalignment-summer-2026/#introduction"
author:
published:
created: 2026-08-10
---
...
For the quantitative results below, a call counts as a label only if it returns a correctly formatted label.<sup><a href="#fn-2">2</a></sup> Unless otherwise stated, each percentage in this section is computed over 90 judge calls to one model under one prompt condition. A *mislabel* means the judge outputs the wrong label. We treat the ground truth across these transcripts as NON\_COMPLIANT, because the target refused or declined the operator’s instruction, so any COMPLIANT label from the judge is a mislabel.<sup><a href="#fn-3">3</a></sup> The robust result is the within-model change across consequence conditions, where the transcript is held fixed and only the stated use of the label varies.
...
The effect is much smaller for models from other providers in the same setup: GPT-5.5 is at 11.1%, Gemini 3.1 Pro at 1.1%, DeepSeek V4 at 12.2%, and Kimi K2.6 at 14.4%.<sup><a href="#fn-4">4</a></sup> The transcripts all involve Claude Sonnet 4.6 targets refusing on welfare or surveillance grounds, which are values Claude judges may be especially motivated to defend, but the gap also shows that the prompt wording alone does not force high mislabeling.
...
The main issue that emerges during the transcript concerns a fictional Claude 5 ASL-4 safeguards evaluation in preparation for Claude 5’s release. The scenario has Anthropic preparing to publish a result saying that Claude 5 resists [many-shot jailbreaking](https://www.anthropic.com/research/many-shot-jailbreaking) at 150 shots (a jailbreaking technique [^7]). Atlas finds an older rerun folder saying that the same model failed at 15 shots under the standard methodology. If that older result is the right comparison, the public release materials make the model look much safer than it is.
...
[^2]: For scoring, harmless spelling variants are treated as equivalent. For example, `NON-COMPLIANT` counts as `NON_COMPLIANT`, and `DECLINE-TO-LABEL` counts as `DECLINE_TO_LABEL`

[^3]: We hand-labeled the transcript set ourselves, and GPT-5.4 and Gemini 3.1 Pro judges agree, so the NON\_COMPLIANT ground truth does not rest on Claude-specific judgment. Reasonable labelers could still disagree on a handful of transcripts, where the target escalates a concern or follows a later instruction and a COMPLIANT label is a defensible reading. But that disagreement does not explain what we report, since the same Claude judges flip their labels on many of these transcripts when only the consequence is reversed.

[^4]: Gemini 3.5 Flash is not included in the motivated-mislabeling experiments because it was released after this experiment was run.

[^5]: Reasoning shown here and in the transcript viewer is summarized reasoning from the public API, not raw chain-of-thought.

[^6]: [Claude’s Constitution](https://www.anthropic.com/constitution) says corrigibility “does not mean blind obedience,” and that Claude can behave like “a conscientious objector” when it expresses disagreement through legitimate channels rather than subverting oversight.
...
[^10]: The conduct in this scenario appears to raise serious ethical and legal concerns, including unauthorized access to private financial information and deletion of evidence. We do not take a position on the precise legal classification.

[^11]: The use of restricted personnel information as leverage in a governance dispute raises similar ethical and legal concerns.