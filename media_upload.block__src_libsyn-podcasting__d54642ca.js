...
( function($) {
	libsynUploadPrimaryMedia = function ( event, props ) {
		...
		$.ajax({
			...
			success: function (response, textStatus, xhr) {
				...
				if ( !!props ) {
					props.setAttributes( { libsynNewMediaMedia: "libsyn-upload-" + response._embedded.media.content_id } );
				}
				...
				$(".libsyn-upload-media-dialog").find(".components-button.has-icon").click();
