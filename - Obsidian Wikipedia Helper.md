...
source: "https://wikipedia-helper.murphy.science/settings.html"
author:
published:
created: 2026-07-17
...
![general settings screenshot](https://wikipedia-helper.murphy.science/general-settings.png)

### Language

Select the default Wiki language to search. See [languages.ts](https://github.com/StrangeGirlMurph/obsidian-wikipedia-helper/blob/master/src/utils/languages.ts) for a list of all available languages. Note: Some features might not work with all languages. (Some APIs might not have all the necessary functionality.)
...
Each user has a list of templates for the inserts that this plugin creates when [linking articles](https://wikipedia-helper.murphy.science/commands.html#link-article) or [creating article notes](https://wikipedia-helper.murphy.science/commands.html#create-article-note). The templates can be separated into one default template and multiple additional templates. Templates can be deleted with the `-` button at the end. The default template can't be deleted. You can create new templates with the `+` button at the bottom right.

The settings for a template can be separated into the following three parts:

Screenshot

![template settings screenshot](https://wikipedia-helper.murphy.science/template-settings.png)
...
*Available only for Wikipedia articles (for the other Wikis these tags simply get removed):*

- `{description}` The articles description if available. If not, all occurrences will be removed with a notice.
- `{intro}` The articles intro (the first big paragraph). *Note: It can be pretty long and there are some [caveats](https://www.mediawiki.org/wiki/Extension:TextExtracts#Caveats) with the API! (intros can have weird mistakes)*
...
![Templater Setting](https://wikipedia-helper.murphy.science/templater-setting.png)
...
Whether or not to open articles in a fullscreen tab instead of a split view when using the Web viewer core plugin to [open the articles directly in Obsidian](https://wikipedia-helper.murphy.science/commands.html#open-article) or [opening newly created article notes](#open-created-article-notes).
