...
created: 2026-07-08
description: "A new machine learning model ranks Telegram posts on their likelihood of containing incidents of civilian harm."
---
...
![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image2.jpg)
...
A semantic similarity score is a calculation used to determine the proximity in meaning between different words and phrases. To get the semantic similarity between the post text and each of our keywords, we represented each in a list of numbers via a [Sentence Transformer model](https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2), which converts words into numerical representations called vectors that a computer can understand.

We then calculated the level of similarity between each vector using [cosine similarity](https://www.geeksforgeeks.org/nlp/different-methods-to-find-document-similarity/), one of the most popular methods for measuring similarity between two pieces of text.

![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image5.jpg)
...
We chose [Logistic Regression](https://en.wikipedia.org/wiki/Logistic_regression) as a baseline algorithm due to its simplicity. We also selected three other “best in class” models, [Random Forest](https://en.wikipedia.org/wiki/Random_forest), [XGBoost](https://xgboost.readthedocs.io/en/release_3.2.0/tutorials/model.html), and [LightGBM](https://en.wikipedia.org/wiki/LightGBM). These choices centred on the [interpretability](https://domino.ai/data-science-dictionary/interpretability) of the models and their ability to work on tabular data of this size. For example, we avoided neural networks due to a lack of interpretability and because those models work best with a larger dataset.
...
We used a [stratified split](https://datature.io/glossary/dataset-splitting) to divide the dataset instead of a random split. This method ensured the proportion of positive instances (i.e. confirmed cases of civilian harm) remained consistent across all three sets at about 11 percent.

![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image8.jpg)
...
There are two main types of evaluation metrics to gauge a model’s prediction power. [Recall](https://en.wikipedia.org/wiki/Precision_and_recall) asserts what fraction of positive instances (i.e. known civilian harm posts) were correctly flagged as such. [Precision](https://en.wikipedia.org/wiki/Precision_and_recall) measures the fraction of posts flagged as civilian harm that are indeed civilian harm posts.

![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image1.jpg)

Walber, CC BY-SA 4.0, via Wikimedia Commons.

During the training phase, we tuned the models to maximise [average precision](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.average_precision_score.html) (PR-AUC), a metric that summarises precision across all recall levels. While this method also accounts for precision, it prioritises recall, which is preferable for this use case as it steers model selection to reduce the number of civilian harm posts that are skipped.
...
![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image4.jpg)
...
![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image3.jpg)
...
- *semantic\_keywords\_similarity*: the semantic proximity between the post text and manually selected keywords “casualties”, “damage” and “civilian harm”
...
- *reaction\_crying\_face*: reactions with crying face emojis on the post
- *group\_of\_messages*: whether a post contains multiple media files
- *keywords\_in\_text*: the number of custom Ukrainian or Russian keywords in the post
...
![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image7.jpg)
...
![](https://www.bellingcat.com/app/uploads/2026/06/civharm_image6.jpg)
...
After selecting this model, we created a user interface where researchers could view a list of Telegram posts sorted from most to least likely to contain indications of civilian harm. The user interface was designed for quick triage and integration, where a positive confirmation from researchers would instantly send the post to the [Auto Archiver](https://www.bellingcat.com/resources/2022/09/22/preserve-vital-online-content-with-bellingcats-auto-archiver-tool/) (Bellingcat’s tool for preserving digital content) and then transfer it to [ATLOS](http://atlos.org/) (our internal collaborative verification platform). Bellingcat staff and volunteers could then manually verify incidents. Researcher input was constantly stored so that this data could be used to improve the model in the future.
