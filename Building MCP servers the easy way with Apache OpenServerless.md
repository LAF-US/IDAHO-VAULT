...
source: "https://opensource.net/building-mcp-servers-with-apache-openserverless/"
author:
  - "[[Michele Sciabarra]]"
  - "[[Bruno Salzano]]"
published: 2025-07-08
created: 2026-05-01
description: "Follow these steps for building an MCP-compliant server using Apache OpenServerless and a custom MCP plugin."
tags:
  - "clippings"
date created: Friday, May 1st 2026, 5:40:52 pm
date modified: Friday, May 1st 2026, 6:06:09 pm
---
...
In this guide, I’ll show how to build an [MCP](https://opensource.net/?s=MCP) -compliant server using Apache OpenServerless and our custom MCP plugin. By deploying OpenServerless and using the plugin, you can quickly find tools via the Model Context Protocol (MCP). This setup enables fast and portable artificial intelligence workflows across any cloud or on-prem environment.
...
This is where OpenServerless with MCP truly shines: enabling fast, portable, and secure AI tool deployment with zero [DevOps](https://opensource.net/?s=+DevOps), seamless orchestration, and full compliance with the Model Context Protocol.

## Introducing olaris-mcp, the OpenServerless plugin to build MCP servers

We developed an Apache OpenServerless plugin, or more precisely an ops plugin for building MCP servers with Apache OpenServerless functions. A quick reminder: ops is the CLI and it supports plugins as a way to extend the CLI of new commands.
...
Important: In OpenServerless, a single MCP server consists of a number of functions, so one single MCP server is a *package*. It consists of a collection of tools, prompts, and resources, each represented as a distinct OpenServerless function. That means one server is always split into a number of microservices.

## Installing the MCP Plugin for OpenServeless
...
Assuming you’ve implemented a get\_weather(location) function, you can now create a wrapper to handle MCP-style invocation:
...
![](https://i0.wp.com/opensource.net/wp-content/uploads/2025/07/mcp_example8E73XTYc1751477562.png?w=512&ssl=1)

In the image above, you can see the result of invoking the weather tool to get weather information for **Rome**.
...
![](https://i0.wp.com/opensource.net/wp-content/uploads/2025/07/mcp_testingTYBK6KAW1751477620.png?w=311&ssl=1)

1. **Ask a Chatbot** Ask a chatbot for the weather in Rome. It will likely reply that, as a language model, it doesn’t have up-to-date weather information.
...
Read part 1: [Apache OpenServerless is the easiest way to build your cloud native AI application](https://opensource.net/apache-openserverless-is-the-easiest-way-to-build-your-cloud-native-ai-application/)