...
It does **not** classify every analyzer-facing file as configuration. In particular, the files literally named `!.js` and `!.ts` at the repository root are controlled experimental artifacts for CodeQL and are documented separately below.
...
The files literally named `!.js` and `!.ts` at the repository root are **not configuration surfaces**. They are pre-existing controlled source artifacts, introduced to test and satisfy CodeQL JavaScript/TypeScript extraction when the repository otherwise had no analyzable source in those languages.
