...
def render_personal_note_markdown(note: PersonalNote) -> str:
    lines = [
        ...
        f"- Institution: `{note.institution.value}`",
        f"- Domain: `{note.domain.value}`",
        f"- Author Kind: `{note.author_kind.value}`",
        f"- Entity: `{note.entity.value}`",
        f"- Personality: `{note.personality.value}`",
        f"- Surface Mode: `{note.surface_mode.value}`",
...
def render_objection_markdown(objection: Objection) -> str:
    lines = [
        ...
        f"- Institution: `{objection.institution.value}`",
    ...
    lines.extend(
        [
            f"- Entity: `{objection.entity.value}`",
            f"- Personality: `{objection.personality.value}`",
            f"- Surface Mode: `{objection.surface_mode.value}`",
...
def render_claim_markdown(claim: Claim) -> str:
    lines = [
        ...
        f"- Institution: `{claim.institution.value}`",
        f"- Domain: `{claim.domain.value}`",
        f"- Entity: `{claim.entity.value}`",
        f"- Personality: `{claim.personality.value}`",
        f"- Surface Mode: `{claim.surface_mode.value}`",
...
def render_council_repo***REMOVED***markdown(report: CouncilReport) -> str:
    lines = [
        ...
        f"- Entity: `{report.entity.value}`",
        f"- Personality: `{report.personality.value}`",
        f"- Surface Mode: `{report.surface_mode.value}`",
...
def render_familiar_gaggle_markdown(note: FamiliarGaggleNote) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Host Entity: `{note.host_entity.value}`",
            f"- Host Personality: `{note.host_personality.value}`",
            f"- Host Surface Mode: `{note.host_surface_mode.value}`",
...
def render_council_session_markdown(session: CouncilSession) -> str:
    ...
    lines = [
        ...
        f"- Institution: `{session.institution.value}`",
        f"- Council Domain: `{session.council_domain.value}`",
        f"- Convener Entity: `{session.convener_entity.value}`",
        f"- Convener Personality: `{session.convener_personality.value}`",
        f"- Convener Surface Mode: `{session.convener_surface_mode.value}`",
...
def render_validation_verdict_markdown(verdict: ValidationVerdict) -> str:
    lines = [
        ...
        f"- Institution: `{verdict.institution.value}`",
        f"- Domain: `{verdict.domain.value}`",
        f"- Entity: `{verdict.entity.value}`",
        f"- Personality: `{verdict.personality.value}`",
        f"- Surface Mode: `{verdict.surface_mode.value}`",
...
def render_gate_repo***REMOVED***markdown(report: GateReport) -> str:
    lines = [
        ...
        f"- Council Entity: `{report.council_entity.value}`",
        f"- Council Personality: `{report.council_personality.value}`",
        f"- Council Surface Mode: `{report.council_surface_mode.value}`",
