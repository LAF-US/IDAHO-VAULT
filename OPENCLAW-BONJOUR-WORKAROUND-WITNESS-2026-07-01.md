...
- OpenClaw's gateway posture on this Mac remains: `bind=loopback`, `tailscale.mode=off`, `mode=local`. Enabling Tailnet exposure for the OpenClaw Android app (per the tiered-capture architecture pinned in `.claude/MEMORY/SESSION-2026-06-29.md`) is downstream work, not affected by the bonjour workaround.
