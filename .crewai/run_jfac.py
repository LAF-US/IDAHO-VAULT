#!/usr/bin/env python3
"""
Run the JFAC Crew — entrypoint script.
...
# Ensure vault root is on the path
vault_root = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(vault_root / ".crewai"))
...
OUTPUT_DIR = vault_root / "!" / "CREWAI"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


def vault_frontmatter(title: str, run_id: str) -> str:
...
def build_jfac_crew() -> Crew:
    """Assemble the JFAC Crew: 3 agents, 5 tasks, sequential process."""
    return Crew(
...
def main():
    ...
    print("=" * 60)
    print(f"JFAC Crew complete. Output: {output_path}")
    return 0
