...
authority: LOGAN
authors: Claude Code (routine CI sweep)
updated: 2026-09-09
...
Everything below this line is the original 2026-08-18 report, kept as the record of that day, with three disclosed amendments: the `pygit2` install command in Incident A, the filename in Incident B, and the root-anchor status in Incident C were corrected in place on 2026-09-09 so the record does not assert things that are no longer true. Every other line stands as written. Read it as a historical snapshot of that day, not as current `main` state — the update above supersedes it.
...
Fixed by renaming, though not by this PR: the rename landed independently on `main` and the file now lives at `CONSISTENT-WITH-NOT-EVIDENCE.md`, a path absent from this PR's final diff. This PR's own first rename dropped only the two illegal quote characters and kept `≠`; `main` later settled the present portable name. Of the first rename: dropped only the two illegal quote characters, kept `≠` (not in the illegal set) and all file content unchanged. Added an `aliases:` frontmatter entry with the original quoted title so existing Obsidian references/search still resolve. Verified clean against `check_portable_paths.py` itself post-rename, and confirmed via a full tracked-tree scan that no other file carries an NTFS-illegal character.
...
- **Root-level `TRIAGEBOT.md`** — missing at the time of this sweep (since authored: present on `main` as of 2026-09-09), and at the time this pushed the script's own root-anchor ratchet from 293 to 294, which the script hard-fails on (`check_dotfolder_anchors.py`'s own docstring: "Effect: existing debt warns; ANY increase fails"). **Not fixed here.** The script's docstring is explicit that a root anchor is "an authored persona note (frontmatter, `related` edges, the chamber's own lines), not a sentinel," and that generating one unprompted would be "an agent writing vault canon it was never asked to write." Confirmed this reproduces identically against current `main` (not introduced by this PR) — `check-dotfolder-anchors` was red on every push/PR against `main` for the duration of this sweep. That is no longer the state: the anchor was authored, and the check passes on `main` as of 2026-09-09.
