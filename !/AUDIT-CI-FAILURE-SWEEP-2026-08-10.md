...
- **Update — mitigation attempted (commit `e3f932e`), and confirmed NOT to have fixed it (run [31411077637](https://github.com/LAF-US/IDAHO-VAULT/actions/runs/31411077637)):** on Logan's "documents the problem but proposes nothing" feedback on #943, sourced the real `.codacy.yml` `exclude_paths` syntax from Codacy's own docs (`docs.codacy.com/repositories-configure/codacy-configuration-file.md`, fetched directly). Added a root `.codacy.yml` with 16 `exclude_paths` entries covering all 21 non-UTF-8 files found above (11 exact filenames + 4 directory/bundle globs — `.serena/**`, `backup-compare-temp/**`, `storage/**`, `LUMO_saint-blue-in-the-desert_20260518.rtfd/**` — covering 4 more files + `minidata*.csv` covering the remaining 6). Correcting an earlier miscount in this same bullet: it previously said "20 paths / 18 files + 3 dirs," which didn't add up; the actual count is 16 config entries covering all 21 files.
  - **Re-ran on the same branch after pushing `.codacy.yml`: identical crash, identical 25-minute timeout, byte-for-byte the same `MalformedInputException` stack trace.** The exclusion had no effect.
  - **Root cause of why it had no effect, confirmed via the verbose CLI log:** line 279 of the job log reads `Success getting config file from endpoint "/project/analysis/configuration"` — the CLI fetched its analysis configuration from **Codacy's remote API**, not from the local `.codacy.yml` in the checkout. `CODACY_PROJECT_TOKEN` is set in `codacy.yml`'s env, and the CLI appears to prefer remote config over a local file when a project token is present. The local `.codacy.yml` this session added was very likely never read.
  - **What that means for the earlier "this file overrides the Codacy UI's Ignored files" warning: it's backwards, or at least unconfirmed** — in this run, remote (dashboard-driven) config took priority, not the local file. Corrected here rather than left standing.
  - **Actual next step, not something this session can complete:** either (a) someone with Codacy dashboard access adds these paths to the project's "Ignored files" settings directly (the mechanism that's actually in effect), or (b) find a CLI flag/env var that forces local-config-only and skips the remote fetch (none was found in the CLI's documented options list during this session), or (c) stop sending `CODACY_PROJECT_TOKEN` to this step — untested and not attempted here, since it likely also controls which tools/severity thresholds run and changing it blind could have side effects beyond this one bug.
- **Not attempted in this session:** deleting/gitignoring the 21 non-UTF-8 tracked files from the repo itself (as opposed to excluding them from Codacy's scan). Several look like accidental commits (backup dumps, stray temp files) rather than intentional vault content, but that determination and any bulk removal is Logan's call, not this routine's, per the vault's no-unauthorized-restructuring rule.
...
| `.codacy.yml` mitigation did not change the outcome | Whether excluding the 21 files would help if the config were actually applied |
| CLI fetched config from Codacy's remote API in this run | Whether that's always true, or only under some condition (e.g. token presence) |
