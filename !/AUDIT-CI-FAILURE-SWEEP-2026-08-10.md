...
- **Update — mitigation attempted (see PR #943):** on Logan's "documents the problem but proposes nothing" feedback on #943, went back and sourced the real `.codacy.yml` `exclude_paths` syntax from Codacy's own docs (`docs.codacy.com/repositories-configure/codacy-configuration-file/`, fetched via WebSearch/curl — confirmed, not guessed, this time). Added a root `.codacy.yml` excluding the 20 non-source paths found above (18 exact non-UTF-8 files + `.serena/**` + `backup-compare-temp/**` + `storage/**`). **This is a best-effort mitigation, not a confirmed fix** — this session has no Docker daemon (`docker info` fails: no socket), so the exact triggering file was never directly reproduced; only the next Codacy run can confirm whether excluding these paths actually clears the crash. Also flagging a real side effect: per Codacy's docs, adding a `.codacy.yml` file **disables whatever "Ignored files" are configured through the Codacy web UI** — whoever has dashboard access should check for UI-side ignores this file might now be silently overriding.
