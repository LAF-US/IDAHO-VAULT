...
2. **Redaction Damage Policy — `agent-git-guardrails` PR (run 28962574565, 2026-07-08T17:29:16Z).** A redaction/sanitization script is clobbering substrings *inside* words across many files (`short_description`, `import_edges`, etc.) instead of whole secret values. **Category: Code bug.** Next step: find and fix the redaction script's matching logic on that branch — it's over-matching word-internal substrings.
