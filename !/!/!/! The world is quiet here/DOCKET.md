...
| GitHub Actions CI diagnosis         | GitHub Copilot | **Resolved** | LAF-7  | Root cause tracked as `LINEAR_API_KEY` configuration gap; workflow now graceful-skips when the secret is unavailable. |
...
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| **`LINEAR_API_KEY` secret**     | Coordination checkpoint: ensure GitHub Actions secret remains present; graceful-skip guard retained for rotation/outage safety. | Logan |
