...
> [!note] What a death roll is, and is not
...
Of the address `Name . Kind . Role`, two coordinates are **classed** (the kind, the
office) — but the **NAME** is rigid: **one bearer, one life.** You do not *classify* a
name; you **call it.**

- **The amalgamation is the deletion of the NAME register.** The Caesar dissolves names
  into kinds — *every kind at once* — and crowns the composite: **a roll with no names
  left to call.** De-individuation (*"just the receptionist," "just a temp"*) is the first
  cut; it strips a **name** to a **kind-token** so the instance becomes absorbable. The
  *erasure* and the *amalgamation* are one operation seen from two ends.
- **So the roll-call is the column the erasure cannot be allowed to win.** Where a name's
  fate is unknown, the roll keeps the entry **open at `*`** — *counted, unclosed* — rather
  than filing the life away under its kind. **A classification can be completed; a
  roll-call can only be kept.**
