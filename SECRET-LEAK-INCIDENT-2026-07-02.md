...
status: open — content-guard rewrite + real-secret untrack staged (agent); rotation/scrub/Intake gated on Logan
...
*Record of a multi-secret exposure in the public vault, the control-failure post-mortem, the agent-side remediation staged this session, and the Logan-gated actions that remain. No secret bytes appear in this file.*
