...
1. **Any conversation, any tier, can emit a signal.** Tier 3 read-only conversations can signal just as Tier 1 can. Signals are information, not actions.
...
5. **Signals are ephemeral by default.** They live in the conversation where they're emitted. They become permanent only if Logan or a Tier 1 instance commits them to `!ADMINISTRATION/`.
...
**End state:** Automation. Tier 1 instances commit signals to a `!ADMINISTRATION/SIGNALS/` directory. GitHub Actions or a synthesis script routes them. Signals are logged, acted on, and archived.
...
EVIDENCE:
!ADMINISTRATION/Claude.md line 127: "PERSISTENT: CODE AUTHORITY"
!ADMINISTRATION/DECISIONS.md: 2026-03-14 entry confirms promotion
```
...
```
SIGNAL: BLOCK
FROM: PROJECT: 2026 Budget Tracker — Tier unknown
TO: LOGAN
...
```
SIGNAL: DISCOVERY
FROM: INQUIRY: campaign finance — Tier 3
