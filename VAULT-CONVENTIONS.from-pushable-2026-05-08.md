...
- **Jupyter Notebook (`.ipynb`)** = executable/literate bridge between Markdown and Python. Use for exploratory analysis, runnable diagnostics, local notebooks, and workflows where the human explanation and machine procedure need to stay adjacent. Every durable notebook should have a paired Markdown note or Markdown index entry that supplies vault metadata, aliases, wikilinks, source context, and governance status.
...
- `.py` for machinery
- `.ipynb` for literate executable work paired to a Markdown note
