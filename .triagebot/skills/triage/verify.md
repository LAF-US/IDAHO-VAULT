...
Verify whether a GitHub issue describes an actual bug or a misunderstanding of intended behavior in IDAHO-VAULT.
...
### Check Source Code/Content for Intent Signals

Look at the relevant files. Pay attention to:

- **Comments or metadata** explaining "why" — strong evidence of intentional design
- **Consistent patterns** across similar files — likely intentional handling
- **File naming conventions** — deliberate organizational choices
...
- A **bug** is when the code/content does something the developer **did not know about or did not choose**. The behavior is accidental.
...
- Falls through by accident (no guard, no comment, no pattern)
...
- Prior issue closed as "not a bug" or similar
...
- **high** — Strong evidence (explicit comments, clear patterns, prior maintainer statements)
