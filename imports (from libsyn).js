...
(function ($) {
	$(document).ready(function() {
		...
		if( !!libsyn_imports.feed_import_triggered ) { //	Only run check before feed import has been triggered to save on resources
			...
			$.getJSON( libsyn_imports.ajax_base_url + '?action=libsyn_check_url&libsyn_check_url=1').done(function(json) {
				if ( !!!json ) {
		...
		if ( !!libsyn_imports.check_import_status ) {
		...
		function showImportSuccess() {
			...
			if ( !!libsyn_imports.data.posts && libsyn_imports.data.posts.length > 1 ) {
				$("#import-feed-status").append("<p style=\"font-weight:bold;\">(" + libsyn_imports.data.posts.length + " Episodes available to be added)</p>")
			}
		}
		...
		function showImportFailed(data={}) {
			...
			try  {
				if ( !!data.error ) {
					...
					$("#feed-import-empty-p").empty().append("Unknown error occured when checking the feed import status.  You may try refreshing the page and trying again in a few minutes.  If this problem persists, please contact us via support@libsyn.com to help troubleshoot this issue.");
				}
			} catch ( error ) {
				$("#feed-import-empty-p").empty().append("Unknown error occured when checking the feed import status.  This was likely due to a timeout issue with your hosting provider.  Many shared hosts set the page timeout to 30 seconds, with larger feed imports this may need to be increased.  For further assistance, please contact us via support@libsyn.com to help troubleshoot this issue.");
