...
| 2026-06-17 | `.github/workflows/batch-arm-merge-queue.yml` schedule + guards | manual-only `workflow_dispatch` -> add `schedule` (`cron: 17 */6 * * *`); arm step fails closed on `armed=0` and validates numeric PR ids — no action version changed | "Engaged by default": now the test runs proved the presser arms (76/76, run `27703050597`), wire it to a 6-hourly rhythm so newly-ready PRs are re-armed without a manual dispatch (arming is idempotent; merge queue + branch protection still gate each PR). Hardening per #548 review: fail closed when PRs existed but none armed (a systemic auth/API failure must not report green — Codex P2); strip trailing CR and skip non-numeric ids before building the PR URL (Copilot). Arming verb, permissions, and pins unchanged | YAML parse; `dry_run=true` dispatch lists PRs; scheduled tick arms ready PRs; required checks | Logan review required |
