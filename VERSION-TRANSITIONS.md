...
| 2026-06-19 | `.github/workflows/auto-merge-engage.yml` (new) | none -> new event-driven workflow (`pull_request_target` + `pull_request_review`), `permissions: contents+pull-requests write` | The wired loop the manual presser never was: per PR event it resolves outdated-resolvable conversations (witnessed `engage-outdated --pr`) and clicks auto-merge (`gh pr merge --auto --merge`), so a PR that satisfies branch protection enters the merge queue with no manual dispatch. Triggered ONLY by native PR events — never downstream of another (possibly disabled) workflow — and it does NOT use the retired risk-tier gate; branch protection (conversations resolved + required reviews/checks) is the sole merge gate, and `needs-fix`/current threads are never touched. `contents: write` for `resolveReviewThread` + `enablePullRequestAutoMerge` (#546 family); checks out the trusted default branch (no PR-authored code run). Reuses the repo-standard checkout/setup-python pins; no action version changed | YAML parse (triggers = pull_request_target + pull_request_review); both composed ops verified working live 2026-06-19 (engage-outdated dispatch ran; batch-arm's `gh pr merge --auto --merge` armed #579/#580, #580 entered the queue); first real PR-event fire confirmed post-merge | Logan review required |
