...
- In Generation I only, depositing into the box did not restore HP/status on withdrawal parity issues aside, the general rule from Gen II forward is that status conditions are effectively cleared on deposit because the box only stores a reduced data structure (Gen III: 80 of 100 bytes) — level, HP, etc. are recalculated on withdraw. [verified] https://bulbapedia.bulbagarden.net/wiki/Pok%C3%A9mon_data_structure_(Generation_III)
