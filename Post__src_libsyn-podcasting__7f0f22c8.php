...
/* 			$attributes = array( 'attributes' =>
			...
			switch ( $postEditorType ) {
				case 'block':
					...
					$localization['libsyn_new_media_media'] = ( !empty(get_post_meta($current_post_id, 'libsyn-new-media-media', true)) ) ? get_post_meta($current_post_id, 'libsyn-new-media-media', true) : '';
					$localization['libsyn_new_media_image'] = ( !empty(get_post_meta($current_post_id, 'libsyn-new-media-image', true)) ) ? get_post_meta($current_post_id, 'libsyn-new-media-image', true) : '';
	...
	public static function postEpisode( $post, $isSchedule=false, $isDraft=false ) {
		...
		if ( $api instanceof \Libsyn\Api ) {
			...
			// if ( $plugin->hasLogger ) $plugin->logger->error(print_r($item));
