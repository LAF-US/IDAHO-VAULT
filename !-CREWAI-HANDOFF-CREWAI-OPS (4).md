...
2. **Dependencies synchronized from the canonical uv lock:**
   ```bash
   uv sync
   ```
   `pyproject.toml` declares direct requirements and `uv.lock` records the resolved graph. `requirements.txt` is an exported pip-compatible artifact, not the primary dependency authority.
