...
| Topology census | Detection, warns | `.github/workflows/sort-audit.yml` ("Vault Topology Census", `workflow_dispatch`-only) running `.github/scripts/topology_census.py` — `sort_audit.py` exists at repo root but no workflow invokes it, despite `VAULT-CONVENTIONS.md`'s automation table listing it on a weekly schedule (stale as of 2026-07-24) |
