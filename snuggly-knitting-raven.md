...
- **SWITCH:** Template goes pure core tokens — remove Templater `undefined` dependency for yesterday/tomorrow
...
Remove all Templater `undefined` syntax from `DAILY NOTE TEMPLATE.md`. Use only core `{{date:}}` tokens. Leave `yesterday:` and `tomorrow:` empty — `periodic-notes` handles date navigation natively, and Linter's `yaml-timestamp` handles `date created`/`date modified`.
