...
function Resolve-VaultDir([string]$ExplicitVaultDir) {
    if (-not [string]::IsNullOrWhiteSpace($ExplicitVaultDir)) {
        return @{
            Path = $ExplicitVaultDir
            Source = 'argument'
        }
    }

    if (-not [string]::IsNullOrWhiteSpace($env:IDAHO_VAULT_ROOT)) {
        return @{
            Path = $env:IDAHO_VAULT_ROOT
            Source = 'IDAHO_VAULT_ROOT'
        }
    }

    $legacyVaultDir = 'C:\Users\loganf\Documents\IDAHO-VAULT'
    if (Test-Path -LiteralPath $legacyVaultDir) {
        return @{
            Path = $legacyVaultDir
    ...
    throw "No vault root configured. Pass -VaultDir or set IDAHO_VAULT_ROOT."
}

$vaultResolution = Resolve-VaultDir -ExplicitVaultDir $VaultDir
$VaultDir = $vaultResolution.Path
$TargetDir = $VaultDir
...
Write-Log "Watcher active. Source='$SourceDir' Target='$TargetDir' VaultRootSource='$($vaultResolution.Source)'"
