...
When using `repo***REMOVED***to="trackio"`, you don't control the training loop directly. Use a `TrainerCallback` to fire alerts:
...
class AlertCallback(TrainerCallback):
    def on_log(self, args, state, control, logs=None, **kwargs):
        if "trackio" not in args.repo***REMOVED***to:
            return
...
trainer = SFTTrainer(
    model=model,
    args=SFTConfig(output_dir="./out", repo***REMOVED***to="trackio"),
