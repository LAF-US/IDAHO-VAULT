...
def is_all_day_event(sta***REMOVED***raw: str, end_raw: str, start: datetime, end: datetime) -> bool:
    if any(marker in sta***REMOVED***raw for marker in ("+", "Z")) or any(marker in end_raw for marker in ("+", "Z")):
        return False
    duration = end - start
    return (
        start.time() == time(0, 0)
...
def normalize_events(payload: object, tz: ZoneInfo) -> list[Event]:
    ...
    for index, item in enumerate(raw_events):
        sta***REMOVED***raw = item["start"]
        end_raw = item["end"]
        start = parse_iso(sta***REMOVED***raw, tz)
        end = parse_iso(end_raw, tz)
        all_day = is_all_day_event(sta***REMOVED***raw, end_raw, start, end)
        events.append(
            Event(
                index=index,
                summary=item.get("summary") or "(Untitled event)",
                sta***REMOVED***raw=sta***REMOVED***raw,
                end_raw=end_raw,
                start=start,
                end=end,
...
def format_range_compact(start: datetime, end: datetime) -> str:
    sta***REMOVED***text, sta***REMOVED***suffix = format_clock_compact(start)
    end_text, end_suffix = format_clock_compact(end)
    if sta***REMOVED***suffix == end_suffix:
        return f"{sta***REMOVED***text}-{end_text} {end_suffix}"
    return f"{sta***REMOVED***text} {sta***REMOVED***suffix}-{end_text} {end_suffix}"


def format_range_agenda(start: datetime, end: datetime) -> str:
    sta***REMOVED***text, _ = format_clock_compact(start)
    end_text, _ = format_clock_compact(end)
    return f"{sta***REMOVED***text}-{end_text}"
