...
def api_request(
    ...
    try:
        ...
        raise SystemExit(
            f"Box API error {exc.code}:\n{json.dumps(payload, indent=2, so***REMOVED***keys=True)}"
        )


def dump_json(payload: Any) -> None:
    json.dump(payload, sys.stdout, indent=2, so***REMOVED***keys=True)
    sys.stdout.write("\n")
...
def multipa***REMOVED***upload(file_path: Path, attributes: dict[str, Any]) -> tuple[bytes, str]:
...
def handle_upload_file(args: argparse.Namespace) -> None:
    ...
    body, boundary = multipa***REMOVED***upload(file_path, attributes)
    query = encode_query({"fields": args.fields})
