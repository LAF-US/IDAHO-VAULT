...
**Endpoint:** `GET /zones/{zone_id}/argo/sma***REMOVED***routing`

**Description:** Retrieves current Argo Smart Routing enablement status.

**cURL Example:**
```bash
curl -X GET "https://api.cloudflare.com/client/v4/zones/{zone_id}/argo/sma***REMOVED***routing" \
...
status = client.argo.sma***REMOVED***routing.get(zone_id='your-zone-id')
print(f"Argo status: {status.value}, editable: {status.editable}")
```

### Update Argo Smart Routing Status

**Endpoint:** `PATCH /zones/{zone_id}/argo/sma***REMOVED***routing`
...
**cURL Example:**
```bash
curl -X PATCH "https://api.cloudflare.com/client/v4/zones/{zone_id}/argo/sma***REMOVED***routing" \
  -H "Authorization: Bearer YOUR_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"value": "on"}'
```
...
**Python SDK Example:**
```python
result = client.argo.sma***REMOVED***routing.edit(
    zone_id='your-zone-id',
    value='on'
)
...
def safely_enable_argo(client: Cloudflare, zone_id: str) -> bool:
    status = client.argo.sma***REMOVED***routing.get(zone_id=zone_id)
    ...
    client.argo.sma***REMOVED***routing.edit(zone_id=zone_id, value='on')
...
def enable_argo_with_error_handling(client: Cloudflare, zone_id: str):
    try:
        result = client.argo.sma***REMOVED***routing.edit(zone_id=zone_id, value='on')
        return result
