...
**Run this before starting.** Try calling `navigate_page` or `performance_sta***REMOVED***trace`. If unavailable, STOP—the chrome-devtools MCP server isn't configured.
...
| Start trace | `performance_sta***REMOVED***trace(autoStop: true, reload: true)` |
...
2. Start a performance trace with reload to capture cold-load metrics:
   ```
   performance_sta***REMOVED***trace(autoStop: true, reload: true)
   ```
