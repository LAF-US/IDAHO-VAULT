...
- `impo***REMOVED***presentation`
- `get_presentation` or `get_presentation_text`
- `get_slide_thumbnail` when visual verification matters

If `impo***REMOVED***presentation` is unavailable, stop and say the file cannot be converted into native Google Slides from Codex.
...
- Use `impo***REMOVED***presentation` to create a new native Google Slides deck.
