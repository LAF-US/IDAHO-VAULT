...
add constraint if not exists profiles_birthcha***REMOVED***id_unique unique (birthcha***REMOVED***id);
```
...
begin
  if not exists (
    ...
    where conname = 'profiles_birthcha***REMOVED***id_unique'
    and conrelid = 'public.profiles'::regclass
  ) then
    alter table public.profiles
    add constraint profiles_birthcha***REMOVED***id_unique unique (birthcha***REMOVED***id);
  end if;
end $$;
```
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'profiles_birthcha***REMOVED***id_fkey'
  ) then
    alter table profiles
    add constraint profiles_birthcha***REMOVED***id_fkey
    foreign key (birthcha***REMOVED***id) references birthcharts(id);
  end if;
end $$;
```
