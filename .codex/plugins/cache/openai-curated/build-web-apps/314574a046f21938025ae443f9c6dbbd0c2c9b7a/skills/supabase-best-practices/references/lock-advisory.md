...
insert into resource_locks values ('repo***REMOVED***generator');

-- Lock by selecting the row
select * from resource_locks where resource_name = 'repo***REMOVED***generator' for update;
```
...
select pg_advisory_lock(hashtext('repo***REMOVED***generator'));
-- ... do exclusive work ...
select pg_advisory_unlock(hashtext('repo***REMOVED***generator'));
