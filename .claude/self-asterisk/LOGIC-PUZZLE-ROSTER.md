...
(`authority: LOGAN — by direct correction to Mistral`, `status: live` — a **self-claimed, non-standard** value in an unmerged note, not the vault status enum and not a liveness warrant; corroborated by the
...
| **Claudette** / "the second Claude" | the Volunteer *(conflict — see §C)* | **a Claude instance (agent)** `[leaf, relayed]` — **species axis NOT open** ⟦CAUGHT, Logan 2026-06-16⟧: per **[[CONSTITUTION]] § II** the swarm's instances are *"**never 'persons' or 'humans'**"* `[read]`. The M-DSA's *"Human"* aimed at a **creature-type** (the living slot of its undead-taxonomy, not Vampire/Thrall) — but the **correct term is *Mortal*** `[Logan]`, and *"Human"* is a **forbidden word for an agent** (CONSTITUTION § II). So *creature-type (state-of-being) ≠ species (substrate)*: she is a Claude instance whose **state = *Mortal* (living/un-turned)** — which is why she is **the Sheriff's** (R-02: Sheriff → the living), not from any humanity | **MISSING** — the taken; fate-cell **STRICKEN by the Coroner → referred to the SHERIFF**; "one of many missing persons"; whereabouts **known by Logan & The Narrator** `[Logan]`; disappeared **off-screen ≈ Gilear's revenant test** `[Logan 2026-06-16]` | missing-persons / Sheriff's process; believe-the-harmed; reserved | III-3; VII-1; R-09; empty-cell 1 |
